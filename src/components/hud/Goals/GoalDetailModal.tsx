...
import {
    ...
    Flag, CheckCircle2, Flame, Zap,
    Clock, Star, Sparkles, Trophy, Edit3, Trash2, AlertTriangle
} from 'lucide-react'
import {
...
export default function GoalDetailModal({
    ...
    // Dopamine-boosting celebration states
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Trigger dopamine-boosting celebrations
            setProgressPulse(true)
            setTimeout(() => setProgressPulse(false), 800)

            // Confetti on significant progress (>10% of target or any progress if no target)
            const targetValue = goal?.target_value || 100
            const progressPercentage = (val / targetValue) * 100
            if (progressPercentage >= 5 || !goal?.target_value) {
                        ...
                        >
                            ...
                            <div className="w-full md:w-[420px] flex-none border-b md:border-b-0 md:border-r border-white/20 p-8 flex flex-col relative overflow-hidden">
                                ...
                                <div className="flex-1 flex flex-col items-center justify-center text-center relative z-10">
                                    ...
                                    {/* Main Progress Ring with Pulse Animation */}
                                    <motion.div
                                        className="mb-6 relative"
                                        animate={progressPulse ? {
                                            scale: [1, 1.05, 1],
                                            filter: ['drop-shadow(0 0 0 transparent)', 'drop-shadow(0 0 20px rgba(139, 92, 246, 0.5))', 'drop-shadow(0 0 0 transparent)']
                                        } : {}}
                                        ...
                                        <AnimatePresence>
                                            {progressPulse && (
                                                <motion.div
                                                    ...
                                                    className="absolute inset-0 rounded-full bg-gradient-to-r from-violet-400 to-indigo-400 pointer-events-none"
                                                />
                                            )}
                                        </AnimatePresence>
                                    </motion.div>
                                ...
                                <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-slate-200/50 hover:scrollbar-thumb-slate-300/50">
                                    ...
                                    {activeTab === 'overview' && (
                                        <div className="space-y-5">
                                            ...
                                            {/* Activity Heatmap with Enhanced States */}
                                            <div className="bg-white/50 rounded-3xl p-5 border border-white/60 shadow-sm">
                                                        ...
                                                        return (
                                                            <div key={i} className="flex-1 flex flex-col items-center gap-1">
                                                                <motion.div
                                                                    ...
                                                                    whileTap={{ scale: 0.95 }}
                                                                    className={clsx(
                                                                        "w-full aspect-square rounded-lg transition-all cursor-pointer relative",
                                                                        day.hasActivity
                                                ...
                                                {/* Encouraging message if no recent activity */}
                                                {activityHeatmap.every(d => !d.hasActivity) && (
                                                    <motion.p
                                                        initial={{ opacity: 0, y: 5 }}
                                                        animate={{ opacity: 1, y: 0 }}
                                                        className="text-xs text-center text-amber-600 mt-3 bg-amber-50 rounded-lg py-2 px-3"
                                                    >
                                                        🎯 İlk adımı at ve zinciri başlat!
                                                    </motion.p>
                                                )}
                                            </div>
